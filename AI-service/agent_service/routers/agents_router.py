...
from services.agent_service.agents import build_private_researcher_agent
...
@agent_router.post("/private_researcher")
async def invoke_private_researcher(state: dict, request: Request):
    
    """
    Invoke the private researcher agent to answer questions about the private researcher in Database (Neo4j and Qdrant).
    ...
    dict
        ...
        - metadata: dict - Any additional metadata
    """
    logger.info("Call private research agent")
    private_researcher_agent = await get_or_create_agent(
        request,
        "private_researcher_agent",
        lambda: build_private_researcher_agent(request.app.state.tool_manager)
    )

    result = await private_researcher_agent.invoke(state, config = {"recursion_limit": 10})
    return result
